...
export default {
  install(app) {
    const componentFiles = import.meta.glob("../views/components/**/*.vue", {
      eager: true,
    });
    const pageFiles = import.meta.glob("../views/pages/admin/**/*.vue", {
      eager: true,
    });
    installComponents(app, componentFiles);
    installComponents(app, pageFiles);
  },
};

function installComponents(app, files) {
