...
@if (false)
    <components-admin-layout current="{{ url()->current() }}" title="{{ $page->title() }}"
        ...
        workspace="{{ route('admin.workspace.show') }}" customers="{{ route('admin.customer.list') }}"
        search="{{ route('admin.search.show') }}"
        keywords="{{ route('admin.keyword.list') }}"></components-admin-layout>
