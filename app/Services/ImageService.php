...
class ImageService
{
    public static function getOptimizedImage(Image $image, $size, $sizes)
    {
        ...
        try {
            ...
            $optimized_image = SpatieImage::load($image->path())->height($size)->width($size)->format('webp')->quality(90)->optimize()->save($path);
        } catch (Exception $e) {
            Log::error($e);
        }
    ...
    public static function getOptimizedImageWH(Image $image, $size, $sizes)
    {
        ...
        try {
            ...
            $optimized_image = SpatieImage::load($image->path())->fit(Fit::Max, $width, $height)->format('webp')->quality(90)->optimize()->save($path);
        } catch (Exception $e) {
            Log::error($e);
        }
